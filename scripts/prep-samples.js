...
function interpolatedVars(str) {
  return Array.from(
    str.matchAll(interpolationRegex),
...
const directories = fs
  ...
  .filter((file) => fs.statSync(path.join(samplesDir, file)).isDirectory());
...
directories.forEach((sample) => {
  ...
  try {
    readme = fs.readFileSync(
      path.join(samplesDir, sample, "README.md"),
      "utf8",
    );
  ...
  let configs = new Set();
  try {
    composeFile = fs.readFileSync(
      path.join(samplesDir, sample, "compose.yaml"),
      "utf8",
    );
    ...
    if (error.code != "ENOENT") {
      console.log(
        `failed to parse compose for configs for sample`,
        sample,
        error,
      );
    }
...
fs.writeFileSync(
  path.join(__dirname, "..", "static", "samples-v2.json"),
  stringified,
);
