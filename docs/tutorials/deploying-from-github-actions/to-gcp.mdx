...
<Tabs>
  <TabItem value="cli" label="gcloud CLI" default>
    You can list all of the projects you have access to with the following
    command: ```bash gcloud projects list --format="value(projectId)" export
    PROJECT_ID="my-project-123456" # export the project ID you want to use ```
    Identify the appropriate project ID from the list and keep it handy for the
    next steps.
  </TabItem>
  <TabItem value="dashboard" label="GCP Console">
    1. Go to the [GCP Console](https://console.cloud.google.com/). 2. Click on
    the project dropdown in the top navigation bar. 3. Identify the appropriate
    project from the list. 4. Your Project ID will be listed next to the project
    name.
  </TabItem>
</Tabs>
