...
| [Azure](/docs/providers/azure) | ✅ Azure Database for PostgreSQL |
...
Managed Postgres is available in all AWS and GCP regions. On Azure, availability varies by region — see [Managed Postgres and Redis region availability](/docs/providers/azure#managed-postgres-and-redis-region-availability) before choosing an `AZURE_LOCATION`.
...
Defang will show a warning when this extension is used on a service that does not use either a `postgres` or `pgvector` image.
