...
```bash
. <(curl -s https://raw.githubusercontent.com/defang-io/defang/bash-install-script/src/bin/install.sh)
```

The script will try to download the appropriate binary for your operating system and architecture, add it to `~/.local/bin`, and add `~/.local/bin` to your `PATH` if it's not already there.
